...
public class RelevantTransition {
	...
	private int cost; //cost of the transition
	...
	public RelevantTransition(String tr_id, String tr_source_state, String tr_target_state, String tr_label, String tr_original_label, int cost) {		
		id = tr_id;
	...
	private void initializePreconditionsAndEffects() {
		...
		if(!source_state.equals(target_state)) {
			PDDL_effects.append("(not (currstate " + source_state + "))" + " (currstate " + target_state + ")");
		}
		else {
			PDDL_effects.append("(currstate " + target_state + ")");
		}
